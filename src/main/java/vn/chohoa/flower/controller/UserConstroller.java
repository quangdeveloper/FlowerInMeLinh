...
import vn.chohoa.flower.jwt.JWTTokenProvider;
...
public class UserConstroller {
    ...
    public ResponseEntity<ResponseDTO> login(
        ...
        /** mỗi lần user đăng sẽ sinh ra 1 token mới nên phai update token nay vào thông tin
...
//        return null;
    ...
    public   ResponseEntity<ResponseDTO> changeAvatarUser(@ModelAttribute("avatar") @Valid UserChangeAvatarDTO avatar) {

        return ResponseEntity.ok().body(
                ResponseDTO.builder()
                        .map(userService.changeAvatar(avatar))
                        .code(Constant.RESPONSE.CODE.OK)
                        .message(Constant.RESPONSE.MESSAGE.OK)
                        .build()
        );
